import { brandsTable, fetchRecords } from '../src/utils/airtable.js';

async function run() {
    const records = await fetchRecords(brandsTable);
    console.log(`Total Brands: ${records.length}`);
    const stats = {};
    ...
    for (const [status, data] of Object.entries(stats)) {
        ...
        samples.forEach(s => {
            console.log(`    ID: ${s.id}, Name: ${s.name || 'N/A'}, Email: ${s.contact_email || 'N/A'}, Created: ${s.created_at || 'N/A'}, Last Activity: ${s.last_activity || 'N/A'}, Activity Log: ${(s.activity_log || '').substring(0, 50)}...`);
        });
    }
