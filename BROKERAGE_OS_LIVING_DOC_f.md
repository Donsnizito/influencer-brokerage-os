...
>
>**Last updated:** 2026-06-10 (Brief 13 done — negotiation extension shipped: prompt extracted to `src/prompts/negotiation_system.md`, `src/lib/email_headers.js` created (header parse utility), `negotiation_handler.js` extended to 5-label taxonomy, chatCompletion tool-use migration (Q5=migrate), three-tier reply matching, deal-range injection, OutreachDrafts reply-write coupling, server.js wired for header extraction. All 8 e2e tests pass. Brief 14 next.)
...
- **2026-06-10** — **Brief 13 done.** Negotiation extension shipped. Four files changed/created: (1) **`src/prompts/negotiation_system.md`** (NEW) — prompt artifact with 5-label taxonomy (`green_yes` | `green_pricing_q` | `yellow_general_q` | `red_pushback` | `red_escalation`), two new RED triggers (`guarantee_scope_expansion` for brand performance-guarantee demands; `payout_structure_pushback` for creator upfront/split-ratio demands), deal-range awareness section (anchor classification to `deal_band_low/high`, `creator_rate_range`, `proposed_rate_band`), 5 few-shots (one per label, spanning both brand and creator senders), operator notes (calibration check, revision discipline, scope distinction from outreach_system.md), Section 1 `<<<`/`>>>` markers and Section 3 JSON block format consumed by the loader. (2) **`src/lib/email_headers.js`** (NEW) — `extractHeaders`, `normalizeMessageId`, `parseReferencesHeader`. Pure string parsing; no negotiation logic; designed for reuse by Briefs 14+. (3) **`src/skills/negotiation_handler.js`** (EXTENDED) — `loadNegotiationPromptArtifact()` at module init (fails loud); `classifyNegotiation()` migrated to `chatCompletion()` tool-use (`classify_reply` tool, Q5=migrate — text-output JSON.parse eliminated); `getDealRangeContext(draftId)` fetches brand.target_budget + creator.rate_range + draft.proposed_rate_band; `findMatchingDraftForReply()` implements three-tier matching; `writeReplyCorrelationToDraft()` is the single reply correlation write path; `processInboundEmail` signature changed to object-parameter `{ record, emailText, senderType, senderEmail, inReplyToHeader, referencesHeader }`. Prior dispatch logic (roster link send on green_yes, quote_terms write on influencer green+rate, flag writes on RED/YELLOW) fully preserved and extended to 5-label taxonomy. (4) **`src/server.js`** (MODIFIED) — added `import { extractHeaders } from './lib/email_headers.js'`; inbound webhook handler extracts `In-Reply-To` and `References` from `fields.headers`; `processInboundEmail` call updated to object-parameter signature. Verification: F.1 module init ✅, F.2 14 header utility unit tests ✅, F.3 8-step e2e ✅ (Tier 1a In-Reply-To match, Tier 1b References chain, Tier 2 most-recent-sender, Tier 3 no-match graceful, guarantee_scope_expansion RED trigger, payout_structure_pushback RED trigger, $25K out-of-band rate detection + rateMentioned=25000, green_pricing_q non-regression). Synthetic test draft created and deleted in try/finally. F.5 TODO/FIXME scan clean. `classifyAndExtract` in `llm.js` now has zero callers; left exported, to be removed in a future cleanup brief. **Brief 14 (match-lock → contract handoff) unblocked.**

- _[next entry: after Brief 14 (match-lock → contract handoff) is shipped]_
